...
exports.DAYS_PER_MONTH = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
exports.CUMULATIVE_MONTH_DAYS = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
exports.CUMULATIVE_MONTH_DAYS_LEAP = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335];
function ex_truncate(t) {
    return (t < 0 ? -1 : 1) * Math.floor(Math.abs(t));
}
exports.ex_truncate = ex_truncate;
function mod1(x, y) {
    return y - ((y - x) % y);
}
exports.mod1 = mod1;
...
exports.jdn_to_ymd = jdn_to_ymd;
function jdn_wday(jdn) {
    return ((jdn + 1) % 7);
}
exports.jdn_wday = jdn_wday;
function jdn_to_iso_wday(jdn) {
    return mod1(jdn_wday(jdn), 7);
}
exports.jdn_to_iso_wday = jdn_to_iso_wday;
function ymd_yday(ymd) {
    var y = ymd.y;
    var m = ymd.m;
    var d = ymd.d;
    if (is_leap_year(y)) {
        return d + exports.CUMULATIVE_MONTH_DAYS_LEAP[m - 1];
    }
    else {
        return d + exports.CUMULATIVE_MONTH_DAYS[m - 1];
    }
}
exports.ymd_yday = ymd_yday;
function ymd_to_quarter(ymd) {
    return Math.floor((ymd.m - 1) / 3.0) + 1;
}
exports.ymd_to_quarter = ymd_to_quarter;
...
exports.ymd_add_years = ymd_add_years;
...
exports.ymd_add_months = ymd_add_months;
...
function iso_weeks_in_year(y) {
    var w = jdn_wday(Math.floor(ymd_to_jdn(new D.YMD(y, 1, 1)).ieEval()));
...
exports.iso_weeks_in_year = iso_weeks_in_year;
