...
export function ex_truncate( t: number): number {
...
export function jdn_to_ymd( jdn: D.ExactRational): D.YMD {
	var x: number = Math.floor( (jdn.ieEval() - 1867216.25) / 36524.25);
...
export function ymd_to_quarter( ymd: D.YMD): number { 
	return Math.floor((ymd.m - 1) / 3.0) + 1;
}

export function ymd_add_years( ymd: D.YMD, n: number): D.YMD {
	var y: number = ymd.y;
	var m: number = ymd.m;
	var d: number = ymd.d;
