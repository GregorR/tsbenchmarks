...
export module Data {
        export class Label {
            ...
            constructor(thing: string, iIn: any = 0, jIn = -1) {
                this.datum = thing;
                this.i = iIn;
                if (jIn == -1) {
                    this.j = thing.length;
        ...
        export class STNode {
            ...
            suffixLink: STNode;
            spID: number;
            constructor(iUpLabel: Label, iParent: STNode, iChildren: STNode[], iSuffixLink: STNode) {
                //
            ...
            removeChildBang(iChild: STNode) {
                let theIndex: any = this.children.indexOf(iChild);
            ...
            upSplit(offset: number) {
                ...
                let newNode: STNode = new STNode(preLabel, parent, [this], undefined);
            ...
            // return both nodes
            upSpliceLeaf(offset: number, leafLabel: any): [STNode, STNode] {
                let splitNode: any = this.upSplit(offset);
                let leaf: any = splitNode.addLeafBang(leafLabel);
                return [splitNode, leaf];
            }
            nodeFollowK(originalLabel: Label, matnK: (a: STNode) => [STNode, number], mateK: (a: STNode, b: number) => [STNode, number], misnK: (a: STNode, b: Label, c: number) => [STNode, number], miseK: (a: STNode, b: number, c: Label, d: number) => [STNode, number]) {
                ...
                let NODEk: any = function (theNode: STNode, label: Label, labelOffset: number): [STNode, number] {
