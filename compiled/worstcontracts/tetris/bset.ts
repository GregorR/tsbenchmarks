...
export module BSet {
        ...
        export function blocksContains(bs: any, b: data.Block): boolean {
            for (var i: any = 0; i < blocksCount(bs); i++) {
        ...
        export function blocksCount(bs: any): any {
            return bs.length;
        }
        // move each block in blockset by diven displacement
        export function blocksMove(dx: any, dy: number, bs: data.Block[]): void {
        ...
        // c: Color, bs: BlockSet
        export function blocksChangeColor(bs: any, c: data.Color): void {
            for (var i: number = 0; i < blocksCount(bs); i++) {
                bs[i] = { x: bs[i].x, y: bs[i].y, color: data.colorCopy(c) };
            }
        }
        // return all blocks in a row
        export function blocksRow(bs: data.Block[], i: number): any {
            var r: data.Block[] = [];
